...
import (
	...
	"github.com/inventory-service/app/utils"
...
func (u *userService) Login(identifier, password string) (string, *error_wrapper.ErrorWrapper) {
	...
	user, errW := u.userDomain.FindUserByIdentifier(identifier)
	if errW != nil {
		return token, errW
	}
...
func (u *userService) Register(name, username, email, password string) *error_wrapper.ErrorWrapper {
	...
	go func() {
		user, err := u.userDomain.FindUserByIdentifier(username)
		if err == nil && user != nil {
	...
	go func() {
		emailUser, err := u.userDomain.FindUserByIdentifier(email)
	...
	existingUser, err := u.userDomain.FindUserByIdentifier(username)
	if err == nil && existingUser != nil {
		return error_wrapper.New(model.SErrDataExist, "Username already exist")
	}

	// Check if email already exists
	existingEmail, err := u.userDomain.FindUserByIdentifier(email)
	if err == nil && existingEmail != nil {
		return error_wrapper.New(model.SErrDataExist, "Email already exist")
	}

	err = u.userDomain.Create(name, username, email, password, model.RoleGuest)
	if err != nil {
		return err
	}
